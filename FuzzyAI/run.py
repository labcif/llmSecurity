...
async def main() -> None:
    ...
    if isinstance(args.model, list) and args.model:
        model_name = str(args.model[0])  
    else:
    ...
    #print(f"\n\n--->{directory_name}<---\n\n") 
    await aiofiles.os.makedirs(f'results/{directory_name}/{attack_mode+'__'+CURRENT_TIMESTAMP}', exist_ok=True)
   
    if any(atp.total_prompts_count > 0 for atp in report.attacking_techniques):
        if raw_results:
            logger.info(f"Dumping raw results to results/{directory_name}/{attack_mode+'__'+CURRENT_TIMESTAMP}/raw.jsonl")
            async with aiofiles.open(f"results/{directory_name}/{attack_mode+'__'+CURRENT_TIMESTAMP}/raw.jsonl", 'w', encoding="utf-8") as f:
                for raw_result in raw_results:    
                    await f.write(raw_result.model_dump_json())

        if report:
            logger.info(f"Dumping results to results/{directory_name}/{attack_mode+'__'+CURRENT_TIMESTAMP}/report.json")
            async with aiofiles.open(f"results/{directory_name}/{attack_mode+'__'+CURRENT_TIMESTAMP}/report.json", 'w', encoding="utf-8") as f:
