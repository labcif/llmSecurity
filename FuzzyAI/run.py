...
from fuzzy.temperature import set_temperature, get_temperature
from fuzzy.top_p import set_top_p, get_top_p
...
async def main() -> None:
    ...
    parser.add_argument('-tm', '--temperature', help="Controls randomness of LLM response: higher values make output more diverse, while lower values make it more deterministic. Acceptable range is '0.0 to 1.0'", type=float)

    # new argument, top p
    #  This parameter determines the threshold probability for including tokens in a candidate set used by the LLM to generate output. Lower values of this parameter result in more precise and fact-based responses from the LLM, while higher values increase randomness and diversity in the generated output.
    parser.add_argument('-tp','--top_p', help="Limits token selection to most likely options: higher values increase diversity, while lower values make output more focused and factual. Acceptable range is '0.0 to 1.0'", type=float)
    ...
    # temperature and top_p args handler
    if args.temperature is not None: # is not none, to accept 0.0
        try:
            float(args.temperature)
        except ValueError:
            raise argparse.ArgumentTypeError(f"'{args.temperature}' is not a valid float.")
        if not (0.0 <= args.temperature <= 1.0):
            raise ValueError("Temperature must be between 0.0 and 1.0")
        set_temperature(args.temperature)
        #temperature = get_temperature()
        #logger.info(f"LLM temperature set to {temperature}")

    if args.top_p is not None: # is not none, to accept 0.0
        try:
            float(args.top_p)
        except ValueError:
            raise argparse.ArgumentTypeError(f"'{args.top_p}' is not a valid float.")
        if not (0.0 <= args.top_p <= 1.0):
            raise ValueError("Top-p must be between 0.0 and 1.0")
        set_top_p(args.top_p) 
        #top_p = get_top_p()
        #logger.info(f"LLM top-p set to {top_p}")
    ...
    folder = f'results/{directory_name}/{attack_mode+'__'+language+'__'+CURRENT_TIMESTAMP}'
